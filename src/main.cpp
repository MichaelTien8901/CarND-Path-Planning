        ...
        if (event == "telemetry") {
          ...
          // j[1] is the data JSON object
            ...
            for(int i = 0; i < sensor_fusion.size(); i++)
            {
              ...
              if (d > (4*lane-0.25) && d < (4*lane + 4+0.25))
              {
...
//            cout << "N= " << N << endl;
            ...
            for(int i = 0; i < 50-prev_path_size; i++)
            {
                ...
                double distance1 = 0;
                double v = car_speed + a * t;
                double MAX_SPEED = 22; // 50MPH
                if ( v < MAX_SPEED ) 
                   distance1 = v * t + 0.5 * a * t * t;
                else {
                   v = MAX_SPEED;
                   distance1 = v * t;
                }
                // follow the lane
                vector<double> new_xy = getXY(car_s + distance1, car_d,  
                  map_waypoints_s, 
                  map_waypoints_x, 
                  map_waypoints_y);
                pos_x = new_xy[0];
